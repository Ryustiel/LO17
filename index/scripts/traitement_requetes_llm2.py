...
# 3. Configure the model for structured output using the Pydantic schema
structured_llm = model.with_structured_output(StructuredQuery)
...
print("LangChain chain created (prompt | structured_llm).")
...
def process_query_to_structured(natural_query: str) -> Optional[dict]:
    ...
    qui utilise le modèle avec .with_structured_output(StructuredQuery)
    pour obtenir directement un objet Pydantic.
    Retourne le résultat sous forme de dictionnaire (avec exclude_none appliqué).
    """
    try:
        result_pydantic: StructuredQuery = chain.invoke({"query": natural_query})

        # Dump the Pydantic object to a dictionary with desired options
        return result_pydantic.model_dump(exclude_none=True)

    except Exception as e:
        print(f"\nErreur lors de l'exécution de la chaîne ou de la génération de sortie structurée : {e}")
        return None
